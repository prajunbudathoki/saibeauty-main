...
import { type SpecialAvailability, getStaffSpecialAvailability } from "@/actions/staff-availability-actions"
import type { StaffSchedule } from "@/actions/staff-schedule-actions"
import { toast } from "sonner"

// Update the props interface to include weekly schedules
interface AvailabilityCalendarProps {
  staffId: string
  locationId: string
  weeklySchedules: StaffSchedule[]
}

// Update the function signature to accept weeklySchedules
export function AvailabilityCalendar({ staffId, locationId, weeklySchedules }: AvailabilityCalendarProps) {
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      setLoading(true)
      try {
        const data = await getStaffSpecialAvailability(staffId, locationId, month, year)
  ...
  const handleDialogClose = (refreshData = false) => {
    ...
    if (refreshData) {
      ...
      getStaffSpecialAvailability(staffId, locationId, month, year)
  ...
  const hasWeeklySchedule = (date: Date) => {
    ...
    return weeklySchedules.some((schedule) => schedule.day_of_week === dayOfWeek)
  }
          ...
          <>
            ...
            <div className="grid grid-cols-7 gap-1">
              ...
              {days.map((day) => {
                ...
                const isAvailable = availability?.is_available ?? hasRegularSchedule
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isAvailable && !isUnavailable ? "bg-green-100 hover:bg-green-200" : ""}
